...
const sanitizeItem = (raw) => {
    ...
    if (trimmed.length > 60)
        throw new Error(`Item "${trimmed.slice(0, 20)}…" is too long.`);
    const cleaned = trimmed.replace(/[^\p{L}\p{N}\s\-']/gu, "");
    if (cleaned.length === 0)
        throw new Error(`Item "${trimmed}" contains no usable characters.`);
    return cleaned;
};
const extractInlineImageData = (response, item) => {
    const candidates = response.candidates ?? [];
    if (candidates.length === 0)
        throw new Error(`No candidates for "${item}".`);
...
app.post('/api/generate', async (req, res) => {
    try {
        ...
        throw new Error(lastError instanceof Error ? lastError.message : "Unknown error generating flashcard.");
    }
    catch (err) {
        console.error(err);
